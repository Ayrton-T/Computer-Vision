...
int main(){
	...
	Mat lena_Laplace2(512, 512, CV_8UC1, Scalar(0));
				...
				for(int a = -1 ; a < 2; a++){
					for(int b = -1 ; b < 2 ; b++){
						if(temp[i+1+a][j+1+b] == -1){
							lena_Laplace2.at<uchar>(i,j) = 0;
							flag = 1;
							break;
						}
					}
					if(flag == 1) break;
					if(flag == 0) lena_Laplace2.at<uchar>(i,j) = 255;
				}
			}
			else lena_Laplace2.at<uchar>(i,j) = 255;
		}
	}

	imshow("lena_Laplace2", lena_Laplace2);
	imwrite("lena_Laplace2.jpg", lena_Laplace2);
	waitKey(0);
	return 0;
}