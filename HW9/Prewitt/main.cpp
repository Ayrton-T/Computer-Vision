...
int main(){
	...
	Mat lena_Prewitt(512, 512, CV_8UC1, Scalar(0));

	for(int i = 0 ; i < x ; i++){
		for(int j = 0 ; j < y ; j++){
			if(temp[i][j] >= threshold) lena_Prewitt.at<uchar>(i,j) = 0;
			else lena_Prewitt.at<uchar>(i,j) = 255;
		}
	}

	imshow("lena_Prewitt", lena_Prewitt);
	imwrite("lena_Prewitt.jpg", lena_Prewitt);
	waitKey(0);
	return 0;
}